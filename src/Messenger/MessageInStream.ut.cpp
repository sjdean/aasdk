    ...
    namespace ut {
      ...
      protected:
        MessageInStreamUnitTest()
            : transport_(&transportMock_, [](auto *) {}), cryptor_(&cryptorMock_, [](auto *) {}),
      ...
      BOOST_FIXTURE_TEST_CASE(MessageInStream_ReceivePlainMessage, MessageInStreamUnitTest)
      {
        ...
        EXPECT_CALL(transportMock_, receive(FrameHeader::getSizeOf(), _)).WillOnce(
            SaveArg<1>(&frameHeaderTransportPromise));
        ...
        EXPECT_CALL(transportMock_, receive(FrameSize::getSizeOf(FrameSizeType::SHORT), _)).WillOnce(
            SaveArg<1>(&frameSizeTransportPromise));
        ...
        EXPECT_CALL(transportMock_, receive(framePayload.size(), _)).WillOnce(
            SaveArg<1>(&framePayloadTransportPromise));
      ...
      BOOST_FIXTURE_TEST_CASE(MessageInStream_ReceiveEncryptedMessage, MessageInStreamUnitTest)
      {
        ...
        EXPECT_CALL(cryptorMock_, decrypt(_, _)).WillOnce(
            DoAll(SetArgReferee<0>(decryptedPayload), Return(decryptedPayload.size())));
        frameSizeTransportPromise->resolve(frameSize.getData());
      ...
      BOOST_FIXTURE_TEST_CASE(MessageInStream_ReceiveSplittedMessage, MessageInStreamUnitTest)
      {
        ...
        transport::ITransport::ReceivePromise::Pointer frameHeaderTransportPromise;
        EXPECT_CALL(transportMock_, receive(FrameHeader::getSizeOf(), _)).Times(2).WillRepeatedly(
            SaveArg<1>(&frameHeaderTransportPromise));
        ...
        EXPECT_CALL(transportMock_, receive(FrameSize::getSizeOf(FrameSizeType::EXTENDED), _)).WillOnce(
            SaveArg<1>(&frame1SizeTransportPromise));
        ...
        EXPECT_CALL(transportMock_, receive(frame1Payload.size(), _)).WillOnce(
            SaveArg<1>(&frame1PayloadTransportPromise));
        ...
        EXPECT_CALL(transportMock_, receive(FrameSize::getSizeOf(FrameSizeType::SHORT), _)).WillOnce(
            SaveArg<1>(&frame2SizeTransportPromise));
        ...
        EXPECT_CALL(transportMock_, receive(frame2Payload.size(), _)).WillOnce(
            SaveArg<1>(&frame2PayloadTransportPromise));
      ...
      BOOST_FIXTURE_TEST_CASE(MessageInStream_IntertwinedChannels, MessageInStreamUnitTest)
      {
        ...
        EXPECT_CALL(receivePromiseHandlerMock_,
                    onReject(error::Error(error::ErrorCode::MESSENGER_INTERTWINED_CHANNELS)));
        EXPECT_CALL(receivePromiseHandlerMock_, onResolve(_)).Times(0);
        frameHeaderTransportPromise->resolve(frame2Header.getData());
