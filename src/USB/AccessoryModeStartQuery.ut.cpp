    ...
    namespace ut {
      ...
      BOOST_FIXTURE_TEST_CASE(AccessoryModeStartQuery_Start, AccessoryModeStartQueryUnitTest)
      {
        ...
        EXPECT_CALL(usbWrapperMock_,
                    fillControlSetup(NotNull(), LIBUSB_ENDPOINT_OUT | USB_TYPE_VENDOR, ACC_REQ_START, 0, 0, 0));

        AccessoryModeStartQuery::Pointer query(
            std::make_shared<AccessoryModeStartQuery>(ioService_, usbWrapperMock_, usbEndpointMock_));
