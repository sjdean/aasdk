    ...
    namespace ut {
      ...
      protected:
        USBEndpointUnitTest()
          ...
          promise_->then(
              std::bind(&USBEndpointPromiseHandlerMock::onResolve, &promiseHandlerMock_, std::placeholders::_1),
      ...
      BOOST_FIXTURE_TEST_CASE(USBEndpoint_ControlTransferForNonControlEndpoint, USBEndpointUnitTest)
      {
        ...
        USBEndpoint::Pointer usbEndpoint(
            std::make_shared<USBEndpoint>(usbWrapperMock_, ioService_, deviceHandle_, 0x01));
      ...
      BOOST_FIXTURE_TEST_CASE(USBEndpoint_BulkTransfer, USBEndpointUnitTest)
      {
        ...
        USBEndpoint::Pointer usbEndpoint(
            std::make_shared<USBEndpoint>(usbWrapperMock_, ioService_, deviceHandle_, endpointAddress));
      ...
      BOOST_FIXTURE_TEST_CASE(USBEndpoint_MultipleBulkTransfers, USBEndpointUnitTest)
      {
        ...
        for (size_t i = 0; i < attemptsCount; ++i) {
          ...
          EXPECT_CALL(usbWrapperMock_,
                      fillBulkTransfer(&transfer, _, endpointAddress, buffer.data, buffer.size, _, _, _))
              .WillOnce(DoAll(SaveArg<5>(&transferCallback), SaveArg<6>(&transfer.user_data)));
      ...
      BOOST_FIXTURE_TEST_CASE(USBEndpoint_InterruptTransfer, USBEndpointUnitTest)
      {
        ...
        common::DataBuffer buffer(data);
        EXPECT_CALL(usbWrapperMock_,
                    fillInterruptTransfer(&transfer, _, endpointAddress, buffer.data, buffer.size, _, _, _))
            .WillOnce(DoAll(SaveArg<5>(&transferCallback), SaveArg<6>(&transfer.user_data)));
        EXPECT_CALL(usbWrapperMock_, submitTransfer(&transfer));
