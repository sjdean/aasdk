  ...
  namespace transport {
    namespace ut {
      ...
      protected:
        TCPTransportUnitTest()
            : receivePromise_(ITransport::ReceivePromise::defer(ioService_)),
              sendPromise_(ITransport::SendPromise::defer(ioService_)), tcpEndpoint_(&tcpEndpointMock_, [](auto *) {}) {
      ...
      BOOST_FIXTURE_TEST_CASE(TCPTransport_ReceiveAtOnce, TCPTransportUnitTest)
      {
        ...
        EXPECT_CALL(tcpEndpointMock_, receive(_, _)).WillOnce(
            DoAll(SaveArg<0>(&dataBuffer), SaveArg<1>(&tcpEndpointPromise)));
      ...
      BOOST_FIXTURE_TEST_CASE(TCPTransport_OnlyOneSendAtATime, TCPTransportUnitTest)
      {
        tcp::ITCPEndpoint::Promise::Pointer tcpEndpointPromise;
        common::DataConstBuffer buffer;
        EXPECT_CALL(tcpEndpointMock_, send(_, _)).Times(2).WillRepeatedly(
            DoAll(SaveArg<0>(&buffer), SaveArg<1>(&tcpEndpointPromise)));
