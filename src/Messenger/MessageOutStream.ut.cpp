    ...
    namespace ut {
      ...
      protected:
        MessageOutStreamUnitTest()
          ...
          sendPromise_->then(std::bind(&SendPromiseHandlerMock::onResolve, &sendPromiseHandlerMock_),
                             std::bind(&SendPromiseHandlerMock::onReject, &sendPromiseHandlerMock_,
                                       std::placeholders::_1));
        }
      ...
      BOOST_FIXTURE_TEST_CASE(MessageOutStream_SendPlainMessage, MessageOutStreamUnitTest)
      {
        ...
        Message::Pointer message(
            std::make_shared<Message>(ChannelId::INPUT, EncryptionType::PLAIN, MessageType::CONTROL));
        message->insertPayload(payload);
        MessageOutStream::Pointer messageOutStream(
            std::make_shared<MessageOutStream>(ioService_, transport_, cryptor_));
      ...
      {
        const FrameHeader
        frameHeader(ChannelId::VIDEO, FrameType::BULK, EncryptionType::ENCRYPTED, MessageType::CONTROL);
        ...
        transport::ITransport::SendPromise::Pointer transportSendPromise;
        EXPECT_CALL(cryptorMock_, encrypt(_, _)).WillOnce(
            DoAll(SetArgReferee<0>(encryptedData), Return(encryptedPayload.size())));
        EXPECT_CALL(transportMock_, send(expectedData, _)).WillOnce(SaveArg<1>(&transportSendPromise));
      ...
      {
        Message::Pointer message(
            std::make_shared<Message>(ChannelId::VIDEO, EncryptionType::PLAIN, MessageType::CONTROL));
        const common::Data payload(1000, 0x5E);
        message->insertPayload(payload);
        MessageOutStream::Pointer messageOutStream(
            std::make_shared<MessageOutStream>(ioService_, transport_, cryptor_));
      ...
      BOOST_FIXTURE_TEST_CASE(MessageOutStream_SendSplittedMessage, MessageOutStreamUnitTest)
      {
        ...
        secondSendPromise->then(std::bind(&SendPromiseHandlerMock::onResolve, &secondSendPromiseHandlerMock),
                                std::bind(&SendPromiseHandlerMock::onReject, &secondSendPromiseHandlerMock,
                                          std::placeholders::_1));
