  ...
  AccessoryModeQueryFactory::createQuery(AccessoryModeQueryType queryType, IUSBEndpoint::Pointer usbEndpoint) {
    switch (queryType) {
      case AccessoryModeQueryType::PROTOCOL_VERSION:
        return std::make_shared<AccessoryModeProtocolVersionQuery>(usbWrapper_, std::move(usbEndpoint));

      case AccessoryModeQueryType::SEND_DESCRIPTION:
        return std::make_shared<AccessoryModeSendStringQuery>(usbWrapper_, std::move(usbEndpoint),
      ...
      case AccessoryModeQueryType::START:
        return std::make_shared<AccessoryModeStartQuery>(usbWrapper_, std::move(usbEndpoint));
