  ...
  namespace usb {
    namespace ut {
      ...
      protected:
        USBHubUnitTest()
            : queryChain_(&queryChainMock_, [](auto *) {}), device_(reinterpret_cast<libusb_device *>(-1)),
              ...
              promise_(IUSBHub::Promise::defer(ioService_)), rawHotplugCallbacHandle_(-1),
      ...
      BOOST_FIXTURE_TEST_CASE(USBHub_QueryDevice, USBHubUnitTest)
      {
        ...
        EXPECT_CALL(usbWrapperMock_,
                    hotplugRegisterCallback(LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED, LIBUSB_HOTPLUG_NO_FLAGS,
        ...
        EXPECT_CALL(usbWrapperMock_, getDeviceDescriptor(device_, _)).WillOnce(
            DoAll(SetArgReferee<1>(connectedDeviceDescriptor), Return(0)));
      ...
      BOOST_FIXTURE_TEST_CASE(USBHub_GetDeviceDescriptorFailed, USBHubUnitTest)
      {
        ...
        libusb_device_descriptor connectedDeviceDescriptor = {0};
        EXPECT_CALL(usbWrapperMock_, getDeviceDescriptor(device_, _)).WillOnce(
            DoAll(SetArgReferee<1>(connectedDeviceDescriptor), Return(1)));
      ...
      BOOST_FIXTURE_TEST_CASE(USBHub_CancelAllQueryChains, USBHubUnitTest)
      {
        ...
        EXPECT_CALL(usbWrapperMock_, getDeviceDescriptor(device_, _)).Times(2).WillRepeatedly(
            DoAll(SetArgReferee<1>(connectedDeviceDescriptor), Return(0)));
        EXPECT_CALL(usbWrapperMock_, open(device_, _)).Times(2).WillRepeatedly(
        ...
        EXPECT_CALL(queryChainMock_, start(deviceHandle_, _)).WillOnce(SaveArg<1>(&queryChainPromise[0])).WillOnce(
            SaveArg<1>(&queryChainPromise[1]));
